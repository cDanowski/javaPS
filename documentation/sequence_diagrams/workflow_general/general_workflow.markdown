...
Details on how the application determines the *binding* from the request are described in the following. First An abstract generic workflow description is presented followed by two concrete examples ([binding for GetCapabilities GET request]()[KVP Binding Example - GetCapabilities GET Request](#kvp-binding-example-for-get-request) and [POX Binding Example - Execute POST Request](#pox-binding-example-for-post-request)).
